...
| Two checks that were not checking | `tsc --noEmit -p tsconfig.json` type-checks nothing when the root config is `files: []` plus references, and `pnpm lint` exited 2 on every invocation because no eslint config existed. Both were in package.json, and one was in CI, presented as checks that were running. |
