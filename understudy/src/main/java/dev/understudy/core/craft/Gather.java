 ...
 * @param findSeconds rough time to get to one, including travel and searching.
 *                    An estimate, and the only guessed number in the model. It
 *                    exists because a plan that treats iron ore and grass as
 *                    equally available is not a plan. Only the ratios matter.
...
public record Gather(String item, int amount, double digSeconds, double findSeconds,
    ...
    public static Gather byHand(String item, double digSeconds, double findSeconds) {
        return new Gather(item, 1, digSeconds, findSeconds, null, 0);
    }

    public static Gather with(String item, double digSeconds, double findSeconds,
                              String tool, int toolUses) {
        return new Gather(item, 1, digSeconds, findSeconds, tool, toolUses);
    }
}
